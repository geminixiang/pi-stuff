...
Live subprocesses are launched and signaled through [Execa 10](https://github.com/sindresorhus/execa/blob/v10.0.1/docs/termination.md), with whole-process-group termination on macOS/Linux (`killDescendants: true`). Output is streamed without Execa buffering into taskd's rotating logs. taskd keeps ownership of durable events, readiness and run-timeout classification, and each stop call's TERM-to-KILL grace period; Execa's fixed-at-launch escalation and exit cleanup are disabled to avoid competing lifecycle owners. Recovery still uses identity-checked group signals because an Execa handle cannot be reattached after a daemon restart. Descendants that create a new process group/session can escape group termination.
