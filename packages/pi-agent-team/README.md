...
- post-report member reflection: after the final report succeeds or fails, every supporting member session receives the settlement and the appropriate public report/outcome, then may propose one compact `when-<critical-situation>-then-<improved-response>.md` lesson. The model supplies only Filename/Trigger/Lesson/Why (and optional evidence); the runtime supplies provenance, timestamps, proposed status, and numeric confidence. Reflection outcomes (`submitted`, `no-lesson`, or sanitized `failed`) are visible per member in the final manifest and do not replace or block the final report;
- member profiles and active lessons resolve project-first from `./.pi/agents/<member-id>/`, then fall back to `~/.pi/agent/agents/<member-id>/`; new proposed lessons are written to the project scope. **Current identity limit:** memory is keyed exactly by `member.id`. Opaque/ephemeral runtime IDs cannot reuse memory across runs. Do not use display names as paths; a future API will separate runtime IDs from stable profile keys;
...
- optional process-local detached execution: `team_start({ detached: true, ... })` returns a `runId` immediately in long-lived TUI/RPC sessions; `team_get` reads a bounded snapshot, `team_wait` waits on `stateChangeSeq` without polling, `team_prompt` intervenes with one member (including unblocking it), and `team_cancel` aborts through a run-owned signal. Lightweight event history is capped at 512 entries per run, at most 64 runs are retained, and events exclude message bodies; runs are cancelled on parent session shutdown and are not persisted across process/session replacement;
...
- Synchronous runs remain foreground and cannot be intervened after their `team_start` call settles. Detached runs and their controls are process/session-local: no daemon, socket API, or restart persistence; parent session shutdown cancels them. A `TeamRuntime` instance still runs exactly once — construct a new one for another run rather than calling `run()` twice.
