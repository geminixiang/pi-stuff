...
export class PiTeamAgent implements TeamAgent {
  ...
  async reflect(
    ...
    signal.addEventListener("abort", onAbort, { once: true });
    try {
      await session.prompt(reflectionPrompt(context));
    } finally {
      signal.removeEventListener("abort", onAbort);
    ...
    if (signal.aborted) throw signal.reason;
    const body = session.getLastAssistantText?.();
    if (!body?.trim()) throw new Error("member produced no reflection response");
    const draft = parseReflection(body);
    if (draft === "no-lesson") return "no-lesson";
    const path = await saveProposedLesson(
      this.cwd,
      this.member.id,
      this.sessionId,
      this.sessionRef,
  ...
  private async getSession(): Promise<SessionLike> {
    ...
    const memberMemory = await loadMemberMemoryPrompt(this.cwd, this.member.id);
  ...
  private queueCommand(command: TeamCommand, confirmation: string) {
    if (this.reporting || this.reflecting)
      return {
        content: [
          {
            type: "text" as const,
            text: this.reporting
              ? "TEAM_SETTLED: coordination tools are inactive during the final report; write the report itself as your response."
              : "TEAM_SETTLED: coordination tools are inactive during reflection; write only the requested reflection format.",
          },
        ],
        details: {},
      };
