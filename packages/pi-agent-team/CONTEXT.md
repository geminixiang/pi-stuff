...
4. Public speech is passive; it enters every member's observation queue but does not itself consume an agent turn. The runtime's `MEMBER_ERRORED` alert and `POLL_CLOSED` result are the only public interrupts.
