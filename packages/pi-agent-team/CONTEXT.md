...
16. `team_wait`, `team_finish`, and `team_claim` end a member's turn: the adapter self-aborts its session immediately after queuing one, so no member can burn its turn budget on repeated same-turn tool calls when there is nothing new to observe.
